...
public class SDFTexture : MonoBehaviour {
    [SerializeField]
    [Tooltip(
        "Either a static 3DTexture asset containing an SDF, or a 3D RenderTexture. A 3D RenderTexture is where MeshToSDF writes the SDF.")]
    Texture m_SDF;

    [SerializeField]
    [Tooltip(
        "Size of the volume. The effective size of the volume will be rounded off to the nearest full voxel, to keep voxels cubic.")]
    Vector3 m_Size = Vector3.one;

    [SerializeField]
    [Tooltip(
        "Voxel count along each axis. Y and Z resolutions are calculated automatically from X and proportions of the volume. Voxel counts above 64^3 might lead to poor performance.")]
    int m_Resolution = 64;
    ...
    public int maxResolution
    {
        get
        {
            ...
            int maxResolution =
                (int)(Mathf.Pow(kMaxVoxelCount * m_Size.x * m_Size.x / (m_Size.y * m_Size.z), 1.0f / 3.0f));
    ...
    private void Update() {
        RenderTexture rt = m_SDF as RenderTexture;
        Texture3D texture3D =
            new Texture3D(rt.width, rt.height, rt.volumeDepth, TextureFormat.RHalf, false);
        RenderTexture.active = rt;
        Graphics.CopyTexture(rt, texture3D); // 这一步拷贝可能无效
        float[,] slice = TextureUtil.slice(texture3D, 0, texture3D.width / 2);
        Debug.Log("");
    }
    ...
    void ValidateTexture() {
        ...
        bool serializedPropertyChanged = rt.depth != 0 || rt.width != res.x || rt.height != res.y ||
                                         rt.volumeDepth != res.z || rt.format != RenderTextureFormat.RHalf ||
