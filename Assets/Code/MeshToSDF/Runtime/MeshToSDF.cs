...
public class MeshToSDF : MonoBehaviour {
    [SerializeField]
    [Tooltip(
        "The SDFTexture that the SDF will be rendered to. Make sure the SDFTexture references a 3D RenderTexture.")]
...
If you need signed distance or just need a limited shell around your surface, use linear flood fill, but many iterations are expensive.")]
    ...
    [SerializeField] UpdateMode m_UpdateMode = UpdateMode.OnBeginFrame;

    public SDFTexture sdfTexture
    {
        get { return m_SDFTexture; }
        set { m_SDFTexture = value; }
    } // 这种写法只是提供了一个外界访问m_SDFTexture的接口
    ...
    [SerializeField] ComputeShader m_Compute = null;
    ...
    public void UpdateSDF(CommandBuffer cmd) {
        if (m_UpdateMode != UpdateMode.Explicit) {
            Debug.LogError("Switch MeshToSDF to explicit scheduling mode before directly controlling its update.",
                this);
            return;
        }
    ...
    void RenderSDF(CommandBuffer cmd) // 指令缓冲区存储了需要执行的指令的列表
    {
        ...
        cmd.BeginSample(Labels.Initialize); // 添加对配置文件采样的命令，主要用于性能分析；Labels中存储的都是String类型的标签
        int kernel = m_InitializeKernel;
        cmd.SetComputeBufferParam(m_Compute, kernel, Uniforms.g_SignedDistanceField, bufferPing);
        cmd.DispatchCompute(m_Compute, kernel, dispatchSizeX, dispatchSizeY, 1); // 添加执行ComputeShader的命令
        ...
        // 添加为ComputeShader设置纹理参数的命令
        // @param computeShader 需要设置参数的ComputeShader
        // @param kernelIndex   需要设置纹理的kernel
        // @param nameID        
        // @param rt            RenderTargetIdentifier，设置的值或标识符
